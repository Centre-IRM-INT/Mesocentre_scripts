...
#SBATCH -J denoiser_ses		# Descriptive job name
...
#list_sub='056 057 061 062 063 064 065 066 067 068 069 070 071 072 073 074 075 076 077 078 079 080 081 082 083 084 085 086 087 088'
...
do
	for ses in $list_ses
	do
		list_func=$(ls $EXPDIR/derivatives/fmriprep/sub-${sub}/ses-${ses}/func/sub-${sub}_ses-${ses}_*space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz)
		...
		do
			...
			regressor_json="$EXPDIR/derivatives/fmriprep/sub-${sub}/ses-${ses}/func/${base_func}_desc-confounds_timeseries.json"
			acompcor_list=$(python get_compcor.py $regressor_json)
			acompcor_list=$(echo "${acompcor_list//"'"/}")
			acompcor_list=$(echo "${acompcor_list//"["/}")
			acompcor_list=$(echo "${acompcor_list//"]"/}")
			acompcor_list=$(echo "${acompcor_list//","/}")
			cosine_list=$(python get_cosine.py "$EXPDIR/derivatives/fmriprep/sub-${sub}/ses-${ses}/func/$regressor_tsv")
...
## default from denoise.sh : csf white_matter std_dvars framewise_displacement trans_x trans_y trans_z rot_x rot_y rot_z 
			...
			python /scratch/jsein/Softs/denoiser/run_denoise.py --col_names $tsv_confounds --out_figure_path $EXPDIR/derivatives/figures_${output_name}  \
		  	$EXPDIR/derivatives/fmriprep/sub-${sub}/ses-${ses}/func/$func \
		  	$EXPDIR/derivatives/fmriprep/sub-${sub}/ses-${ses}/func/$regressor_tsv \
		  	$EXPDIR/derivatives/$output_name  \
