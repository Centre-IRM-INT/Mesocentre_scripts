...
for i in js.keys():
    if i.startswith('a_comp_cor'):
        if js[i]['Mask'] == 'CSF' and js[i]['CumulativeVarianceExplained'] < 0.5:
                a_comp_cor_csf.append(i)
                
        if js[i]['Mask'] == 'WM' and js[i]['CumulativeVarianceExplained'] < 0.5:
...
a_comp_cor = a_comp_cor_csf[:] + a_comp_cor_wm[:]
